	...
	$(document).on('click','.leadScript',function(){
	 ...
	 $('.sublead_insert').click(function(event){
		...
		if($('.expected_start_datetime').val()==''){
			 Add_Error($('.expected_start_datetime'),'This Field is Required!');
			 event.preventDefault();
		}else{
			Remove_Error($('.expected_start_datetime'));
		}
		if($('.expected_end_datetime').val()==''){
			 Add_Error($('.expected_end_datetime'),'This Field is Required!');
			 event.preventDefault();
		}else{
			Remove_Error($('.expected_end_datetime'));
		}
...
<div class="panel panel-info">
	<div class="panel-heading">
    	<h3 class="panel-title">Lead View - <?= $model->lead_id ?>
    ...
    <div class="tabbable">
        ...
        <li class="active"><a href="#desc" role="tab" data-toggle="tab">Lead Description</a></li>
        <li><a href="#attachments" role="tab" data-toggle="tab">Attachments	
           
            <span class="badge"> <?= FileModel::getAttachmentCount('lead',$model->id)?></span>
        </a></li>
        <li><a href="#notes" role="tab" data-toggle="tab">Notes</a></li>
        <li><a href="#subleadtab" role="tab" data-toggle="tab">Sub Lead
        	<span class="badge"><?= LeadSearch::getSubLeadCount($model->id)?></span>
        	 
        </a></li>
        <li><a href="#timesheet" role="tab" data-toggle="tab">Timesheet</a></li>
        <li><a href="#assign_history" role="tab" data-toggle="tab">Assignment History</a></li>
        <!--<li><a href="#history" role="tab" data-toggle="tab">History</a></li>-->
        <li><a href="#history" role="tab" data-toggle="tab">Activity</a></li>
        </ul>
    ...
    echo Html::submitButton ( $model->isNewRecord ? 'Create' : 'Update', [ 
                    ...
                    ] );?> <a href="javascript:void(0)" class="btn btn-success btn-sm" onClick="$('.add-notes-modal').modal('show');"><i class="glyphicon glyphicon-comment"></i> New Note</a>
                    <a href="javascript:void(0)" class="btn btn-success btn-sm" onClick="$('.savepopup').modal('show');"><i class="glyphicon glyphicon-save"></i> New Attachment</a>
                    <a href="javascript:void(0)" class="btn btn-success btn-sm leadScript" onClick="$('.leadae').modal('show');"><i class="glyphicon glyphicon-tag"></i> Add Sublead</a>
                     
                     <?php
                    if(!empty($_COOKIE['lead_start_time'])){
                        if($_COOKIE['leadStartedId']==$_GET['id']){
                    ?>
                    <a href="javascript:void(0)" class="btn btn-sm   btn-danger <?=$_COOKIE['lead_start_time']?'':'hideBtn'?> stopTime"  data-toggle="modal"><i class="glyphicon glyphicon-time"></i> End Timer</a>
                    <?php 
                        }
                    }else{?>
                        <a href="index.php?r=sales/lead/lead-view&id=<?=$_GET['id']?>&starttime=true" class="btn btn-sm  btn-success <?=$_COOKIE['lead_start_time']?'hideBtn':''?>"><i class="glyphicon glyphicon-time"></i> Start Timer</a>
                    <?php }?>
