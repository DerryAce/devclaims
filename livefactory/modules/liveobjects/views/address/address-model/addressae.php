...
<?php Yii::$app->request->enableCsrfValidation = true; ?>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
     ...
     ArrayHelper::map(State::find()->where('id=0')->orderBy('state')->asArray()->all(), 'id', 'state'), ['prompt' => '--Select--','class'=>'form-control','id'=>'sub_state_id','data-validation'=>'required' ]  ).'</div></div>
     ...
     ArrayHelper::map(City::find()->where('id=0')->orderBy('city')->asArray()->all(), 'id', 'city'), ['prompt' => '--Select--','class'=>'form-control','id'=>'sub_city_id' ]  ).'</div></div></div>';
      ...
      <div class="modal-footer">
      	...
      	<button type="submit" class="btn btn-primary add_address">
