...
$('#task-actual_end_datetime').change(function(){
	...
	//	alert(startTime>endTime);
		...
		if(startTime>endTime){
			 ...
			 Add_Error($('#task-actual_end_datetime-disp'),'Start Time Should be Less then Completion Time');
	...
	if($('#task-task_status_id').val() ==2){
			...
			if('<?=!empty($_COOKIE['inserted_notes'.$model->id])?$_COOKIE['inserted_notes'.$model->id]:''?>' != '1'){
				...
				if(startTime>endTime){
					 ...
					 error+=Add_Error($('#task-actual_end_datetime-disp'),'Start Time Should be Less then Completion Time');
				...
				if($('#task-actual_start_datetime-disp').val() ==''){
					...
					error+=Add_Error($('#task-actual_start_datetime-disp'),'This field Required!');
				...
				if($('#task-actual_start_datetime-disp').val() ==''){
					...
					error+=Add_Error($('#task-actual_end_datetime-disp'),'This field Required!');
...
<div class="panel panel-info">
	...
	<div class="panel-heading">
    	...
    	<h3 class="panel-title"><?= Yii::t('app', 'Task View')?> - <?= $model->task_id ?>

        	<div class="pull-right">

            	<?php if($model->parent_id){?>

            	<a href="index.php?r=pmt/task/task-view&id=<?=$model->parent_id?>" class="btn btn-xs btn-default"><span class="glyphicon glyphicon-new-window"></span> <?= Yii::t('app', 'View Parent Task')?></a>&nbsp;

                <?php } ?>

                <a href="index.php?r=pmt/project/project-view&id=<?=$model->project_id?>" class="btn btn-xs btn-default"><span class="glyphicon glyphicon-new-window"></span> <?= Yii::t('app', 'View Project')?></a>&nbsp;

                <a class="close" href="index.php?r=pmt/task/index" >
    ...
    <div class="tabbable">
        ...
        <li><a href="#subtasktab" role="tab" data-toggle="tab"><?= Yii::t('app', 'Sub Tasks')?>
    ...
    echo Html::submitButton ( $model->isNewRecord ? Yii::t('app', 'Create' ): Yii::t('app', 'Update'), [ 
                    ...
                    ] );?> <a href="javascript:void(0)" class="btn btn-success btn-sm" onClick="$('.add-notes-modal').modal('show');"><i class="glyphicon glyphicon-comment"></i> <?= Yii::t('app', 'View Project')?><?= Yii::t('app', 'New Note')?></a>
